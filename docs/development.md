...
| [`monitor`](https://github.com/Kidev/SynQt/tree/main/tests/monitor)                | The event pipeline every entity carries: the record, the bounded ring that drops the oldest and counts what it dropped, the per category levels, and the writer thread that batches off the caller's thread. It links Qt Core and Qt Test and nothing else, which is what keeps the pipeline out of the GPLv3 libraries. |
