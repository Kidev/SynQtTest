...
tools/          The command line tooling: the CLI, the contract generator, the docs lexer.
cmake/          SynQtContracts.cmake: the .syn -> rep -> repc + QML registration glue.
tests/          One self contained CMake project per milestone and per acceptance fixture.
benchmarks/     The performance harnesses and their committed baselines.
examples/       The materialized tutorial systems (gavel, the auction; arena, the game).
...
`src/`. This mirrors how a real SynQt project is laid out: entities are separate targets
...
|------------------|------------------|--------------------------------------------------------------------|----------------|
| `SynQtTransport` | `src/transport`  | Qt Core, WebSockets                                                | `WebSocketTransport`: the `QIODevice` over a `QWebSocket` that carries QtRemoteObjects. Shared by both the client and the web edge, so it is its own leaf library with no client or service dependency. |
| `SynQtClient`    | `src/client`     | Qt Core, Network, WebSockets, RemoteObjects, Qml, Quick            | The client runtime: `SynClient` (the wss connection and reconnection), `ServerAccessor` (the `Server` QML accessor), `Session`, `Router`, the typed `ReplicaRegistry`, and client logging. Links into both the WebAssembly and the native desktop client. |
...
| `SynQtService`   | `src/service`    | Qt Core, Network, NetworkAuth, Qml, RemoteObjects, WebSockets, HttpServer, OpenSSL, jwt-cpp | Everything a service entity needs: `EntityRuntime` and `ConnectPointHost` (topology and hosting), the mesh transport (`MeshServer`, `MeshClient`, `MeshPeer`), the `WebEdge` (HTTP bundle serving and the WebSocket upgrade pipeline), `SessionManager` and `Caller`, and the identity stack (`IdentityProvider`, `OAuthBackend`, `JwksVerifier`, the identity service and its dev stub). |
...
- `tools/synqtc` is the contract generator. It parses a `.syn` contract (`parser.py`,
...
- `tools/synqt` is the `synqt` command line tool. Each subcommand is its own module:
...
- `tools/pygments-synqt` is the Pygments lexer that colours SynQt flavoured QML in the
...
|--------------------------|----------------|
| `m0-transport`           | QtRemoteObjects over QtWebSockets works in a real browser (the go or no go gate). Driven by the Playwright verifier, also run by `browser-matrix.yml`. |
| `m1-contract`            | `.syn` lowers to the correct rep with push properties and role limited models. |
| `m2-transport`           | The `WebSocketTransport` carries a replica over a real WebSocket. |
| `m3-mesh`                | Mesh mutual TLS by default, plus the opt in local socket, with wrong or missing certificates rejected at the handshake. |
| `m4-topology`            | The entity runtime resolves the topology and refuses a link that is not declared (deny by default). |
| `m5-webedge`             | The web edge serves the bundle with the right headers and runs the upgrade verifier before a socket exists. |
| `m6-client`              | The client runtime and the counter example, synced across two clients. |
| `m6-clientupdate`        | The `App` accessor: an update no one handles reloads immediately, an app that handles `App.onUpdateReady` owns the timing, and the attached-handler syntax resolves in real QML. |
| `m7-caller`              | Sessions, scopes, and the `Caller` accessor, on the three entity todo authorization matrix. |
| `m8-auth`                | Provider login, the browser holding only a session cookie, and tokens never leaving the edge. |
| `m9-providers`           | The persistence and cache providers behind their interfaces, injection safety, and write serialization. |
| `prov4-runtime`          | The entity runtime injects the configured provider into a blueprint entity, and refuses to start when the provider cannot be built. |
...
| `consumer-facade`        | The `Contract.on<Signal>` handlers and the returning slot promise. |
| `fix1-auction`           | The auction tutorial as an acceptance fixture. |
| `fix2-arena`             | The multiplayer arena tutorial as an acceptance fixture. |
| `appgen-native`          | The generated CMake and mains actually compile for every entity. |
| `desktop-client`         | The native desktop client target compiles, installs, and boots. |
...
under `benchmarks/results/`, keyed by hostname, so a committed baseline fails review when a
change regresses it. `benchmarks/README.md` describes each harness and how to run it,
including the ones that need a real display or a non sandboxed host.

## The documentation site (`docs/`)

The site is MkDocs with the Material theme, configured in `mkdocs.yml`. `overrides/`
...
in `tools/pygments-synqt` colours the code samples. It is built and published by
`docs.yml` on a push to `main`.

## Continuous integration (`.github/workflows/`)

The workflows are described in [build system and CLI](build-system-and-cli.md#continuous-integration).
In short: `tests.yml` runs the Python suites on Linux, macOS, and Windows; `ctest.yml`
provisions the pinned Qt kit through aqtinstall and runs the native C++ suites;
`browser-matrix.yml` runs the M0 transport proof across Chromium, Firefox, and WebKit;
...
`synqt build` of the arena's client bundle); `release.yml` freezes and publishes the CLI; and
`docs.yml` publishes this site.

`browser-matrix.yml` and `wasm-proofs.yml` are `workflow_dispatch`-first and also run on
changes to what they cover: each builds a Qt module from source for the WebAssembly kit (which
ships no QtRemoteObjects, see `tests/m0-transport/README.md`), so they are too slow for every
...
terms are in the repository's `CONTRIBUTING.md`.
