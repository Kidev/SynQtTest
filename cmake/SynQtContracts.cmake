...
function(synqt_add_contract target)
    ...
    foreach(syn IN LISTS ARG_SYN)
        ...
        # repc names its output after the .rep's own basename, and writes it into the
        # DIRECTORY's binary dir rather than the target's; moc then names its bookkeeping
        # after that. Two targets in one directory generating the same contract at the same
        # role therefore write the same files, and CMake refuses the whole project with
        # "Files to be generated by multiple different commands". That is not an exotic
        # topology: it is one connect point with two consumers, which is what you have the
        # moment a gateway and a web edge read the same table.
        #
        # So the copy repc is given carries the target's name, and every output derived from
