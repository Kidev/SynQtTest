...
def _tests_cmake(config: Dict[str, Any], qt_version: str,
    ...
    return lines + [
        ...
        '    set(SYNQT_APP_ROOT "${CMAKE_CURRENT_SOURCE_DIR}")',
        # build/generated/ is under the project, not under this configure's binary dir:
        # `synqt build` writes it once and every preset (host, wasm, a user's own) reads
        # the same copy.
        '    add_subdirectory("${CMAKE_CURRENT_SOURCE_DIR}/build/generated"'
...
def render_tests_cmakelists(config: Dict[str, Any]) -> str:
    """The generated `build/generated/CMakeLists.txt` defining the app test target.

    It reads `SYNQT_APP_ROOT` from the parent scope for the paths that are in the source
    tree (the contracts and the tests themselves), because its own source directory is
...
def _client_cmake(config: Dict[str, Any], client: Dict[str, Any], uri: str,
    ...
    qml_files = ['"${CMAKE_CURRENT_SOURCE_DIR}/%s/%s"' % (folder, view) for view in views]
