...
// Fetch the bundle the caller has just become entitled to, while they are still looking
...
def render_warm_script() -> str:
    ...
    What it does not do, deliberately: it does not fill the service worker's cache. That
    precache fetches with `cache: "reload"` on purpose (see render_service_worker_js), so
