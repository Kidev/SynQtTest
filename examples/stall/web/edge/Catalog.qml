...
// reach the stock entity directly, so the edge holds this live list of offers and fills it from
// the stock entity's own table. It mirrors the stock entity's `itemStocked` signal into the `offers`
// model with setOffers, which keeps only the declared roles: the internal sku the stock entity
