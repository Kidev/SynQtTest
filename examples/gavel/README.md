...
| File | Tutorial page |
| --- | --- |
| `web/edge/Auction.syn`, `web/edge/Auction.qml`, `client/app/Main.qml` | [The base case](../../docs/tutorial-base-auction.md) |
| `web/edge/identity/map.qml`, `web/edge/.env.example`, `identity:` in `synqt.yaml` | [Real bidders](../../docs/tutorial-sign-in.md) |
| `db/relational/books/Ledger.syn`, `web/edge/Hall.syn`, `db/relational/books/Ledger.qml`, `db/relational/books/schema.sql`, `web/edge/Hall.qml` | [A permanent Hall of Fame](../../docs/tutorial-hall-of-fame.md) |
...
The server-side Sources here (`web/edge/Auction.qml`, `web/edge/Hall.qml`, `db/relational/books/Ledger.qml`) use
the framework's owner API. `web/edge/Auction.qml` answers one caller with the typed sugar
...
works and is what `web/edge/Hall.qml` uses for a model it publishes with `set<Model>(rows)`).
